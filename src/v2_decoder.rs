...
//     return 0;
// }
pub fn v2d_flags_ready(decoder: &mut ZmqDecoder, buf: &[u8]) -> i32 {
    ...
    if (decoder._tmpbuf[0] & LARGE_FLAG) {
        decoder.next_step(decoder._tmpbuf, 8, v2d_eight_byte_size_ready);
    } else {
        decoder.next_step(decoder._tmpbuf, 1, v2d_one_byte_size_ready);
    }
...
// }
pub fn v2d_one_byte_size_ready(decoder: &mut ZmqDecoder, read_from_: &[u8]) -> i32 {
    return v2d_size_ready(decoder._tmpbuf[0], read_from_);
}
...
// }
pub fn v2d_eight_byte_size_ready(decoder: &mut ZmqDecoder, read_from_: &[u8]) -> i32 {
    //  The payload size is encoded as 64-bit unsigned integer.
    //  The most significant byte comes first.
    let msg_size = get_u64(decoder._tmpbuf);

    return v2d_size_ready(msg_size, read_from_);
}
...
pub fn v2d_size_ready(decoder: &mut ZmqDecoder, msg_size_: u64, read_pos_: &[u8]) -> i32 {
    //  Message size must not exceed the maximum allowed size.
    if (decoder._max_msg_size >= 0) {
        if ((msg_size_ > (decoder._max_msg_size))) {
            // errno = EMSGSIZE;
            return -1;
        }
    }
    ...
    let rc = decoder._in_progress.close();
    // assert (rc == 0);
    ...
    if ((!decoder._zero_copy || msg_size_ > (
        allocator.data() + allocator.size() - read_pos_))) {
        // a new message has started, but the size would exceed the pre-allocated arena
        // this happens every time when a message does not fit completely into the buffer
        rc = decoder._in_progress.init_size((msg_size_));
    } else {
        // construct message using n bytes from the buffer as storage
        // increase buffer ref count
        // if the message will be a large message, pass a valid refcnt memory location as well
        rc = decoder._in_progress.init((read_pos_),
                                    (msg_size_),
                                    call_dec_ref,
                                    allocator.buffer(), allocator.provide_content());
    ...
    // to the current start address in the buffer because the message
    // was constructed to use n bytes from the address passed as argument
    decoder.next_step(decoder._in_progress.data_mut(), decoder._in_progress.size(),
                   v2d_message_ready);
...
//     return 1;
// }
pub fn v2d_message_ready(decoder: &mut ZmqDecoder, buf: &[u8]) -> i32 {
    //  Message is completely read. Signal this to the caller
    //  and prepare to decode next message.
    decoder.next_step(decoder._tmpbuf, 1, v2d_flags_ready);
    return 1;
}
