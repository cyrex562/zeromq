...
static u32 L32(u32 x,c: i32) { return (x << c) | ((x&0xffffffff) >> (32 - c)); }
...
static int vn(const u8 *x,const u8 *y,n: i32)
{
  u32 i,d = 0;
  FOR(i,n) d |= x[i]^y[i];
  return (1 & ((d - 1) >> 8)) - 1;
...
static u64 R(u64 x,c: i32) { return (x >> c) | (x << (64 - c)); }
