...
use crate::socket::dgram::{dgram_xattach_pipe, dgram_xgetsockopt, dgram_xhas_in, dgram_xhas_out, dgram_xjoin, dgram_xpipe_terminated, dgram_xread_activated, dgram_xrecv, dgram_xsend, dgram_xsetsockopt, dgram_xwrite_activated};
use crate::socket::dish::{dish_xattach_pipe, dish_xgetsockopt, dish_xhas_in, dish_xhas_out, dish_xjoin, dish_xpipe_terminated, dish_xread_activated, dish_xrecv, dish_xsend, dish_xsetsockopt, dish_xwrite_activated};
use crate::socket::gather::{gather_xattach_pipe, gather_xgetsockopt, gather_xhas_in, gather_xhas_out, gather_xjoin, gather_xpipe_terminated, gather_xread_activated, gather_xrecv, gather_xsend, gather_xsetsockopt, gather_xwrite_activated};
use crate::socket::pair::{pair_xattach_pipe, pair_xgetsockopt, pair_xhas_in, pair_xhas_out, pair_xjoin, pair_xpipe_terminated, pair_xread_activated, pair_xrecv, pair_xsend, pair_xsetsockopt, pair_xwrite_activated};
use crate::socket::peer::{peer_xattach_pipe, peer_xgetsockopt, peer_xhas_in, peer_xhas_out, peer_xjoin, peer_xpipe_terminated, peer_xread_activated, peer_xrecv, peer_xsend, peer_xsetsockopt, peer_xwrite_activated};
use crate::socket::pull::{pull_xattach_pipe, pull_xgetsockopt, pull_xhas_in, pull_xhas_out, pull_xjoin, pull_xpipe_terminated, pull_xread_activated, pull_xrecv, pull_xsend, pull_xsetsockopt, pull_xwrite_activated};
use crate::socket::push::{push_xattach_pipe, push_xgetsockopt, push_xhas_in, push_xhas_out, push_xjoin, push_xpipe_terminated, push_xread_activated, push_xrecv, push_xsend, push_xsetsockopt, push_xwrite_activated};
use crate::socket::radio::{radio_xattach_pipe, radio_xgetsockopt, radio_xhas_in, radio_xhas_out, radio_xjoin, radio_xpipe_terminated, radio_xread_activated, radio_xrecv, radio_xsend, radio_xsetsockopt, radio_xwrite_activated, UdpPipes};
use crate::socket::rep::{rep_xattach_pipe, rep_xgetsockopt, rep_xhas_in, rep_xhas_out, rep_xjoin, rep_xpipe_terminated, rep_xread_activated, rep_xrecv, rep_xsend, rep_xsetsockopt, rep_xwrite_activated};
use crate::socket::req::{req_xattach_pipe, req_xgetsockopt, req_xhas_in, req_xhas_out, req_xjoin, req_xpipe_terminated, req_xread_activated, req_xrecv, req_xsend, req_xsetsockopt, req_xwrite_activated};
use crate::socket::router::{router_xattach_pipe, router_xgetsockopt, router_xhas_in, router_xhas_out, router_xjoin, router_xpipe_terminated, router_xread_activated, router_xrecv, router_xsend, router_xsetsockopt, router_xwrite_activated};
use crate::socket::scatter::{scatter_xattach_pipe, scatter_xgetsockopt, scatter_xhas_in, scatter_xhas_out, scatter_xjoin, scatter_xpipe_terminated, scatter_xread_activated, scatter_xrecv, scatter_xsend, scatter_xsetsockopt, scatter_xwrite_activated};
use crate::socket::server::{server_xattach_pipe, server_xgetsockopt, server_xhas_in, server_xhas_out, server_xjoin, server_xpipe_terminated, server_xread_activated, server_xrecv, server_xsend, server_xsetsockopt, server_xwrite_activated};
use crate::socket::sub::{sub_xattach_pipe, sub_xgetsockopt, sub_xhas_in, sub_xhas_out, sub_xjoin, sub_xpipe_terminated, sub_xread_activated, sub_xrecv, sub_xsend, sub_xsetsockopt, sub_xwrite_activated};
use crate::socket::xpub::{xpub_xattach_pipe, xpub_xgetsockopt, xpub_xhas_in, xpub_xhas_out, xpub_xjoin, xpub_xpipe_terminated, xpub_xread_activated, xpub_xrecv, xpub_xsend, xpub_xsetsockopt, xpub_xwrite_activated};
use crate::socket::xsub::{xsub_has_out, xsub_xattach_pipe, xsub_xgetsockopt, xsub_xhas_in, xsub_xjoin, xsub_xpipe_terminated, xsub_xread_activated, xsub_xrecv, xsub_xsend, xsub_xsetsockopt, xsub_xwrite_activated};
use crate::socket::zmq_pub::{pub_xattach_pipe, pub_xgetsockopt, pub_xhas_in, pub_xhas_out, pub_xjoin, pub_xpipe_terminated, pub_xread_activated, pub_xrecv, pub_xsend, pub_xsetsockopt, pub_xwrite_activated};
...
impl<'a> ZmqInprocs<'a> {
    ...
    pub fn erase_pipes(&mut self, ctx: &mut ZmqContext, endpoint_uri_str: &str) -> Result<(), ZmqError> {
        for (k, v) in self._inprocs.iter_mut() {
            if k == endpoint_uri_str {
                v.send_disconnect_msg(ctx);
                v.terminate(ctx, true)
            }
        }
...
pub struct ZmqSocket<'a> {
    ...
    pub monitor_socket: Option<&'a mut ZmqSocket<'a>>,
...
impl<'a> ZmqSocket<'a> {
    ...
    pub fn get_peer_state(&mut self, routing_id: *const c_void, routing_id_size_: usize) -> Result<(), ZmqError> {
        unimplemented!()
    }
    ...
    pub fn setsockopt(&mut self, options: &mut ZmqOptions, option: i32, optval: &mut [u8], optvallen: usize) -> Result<(), ZmqError> {
        ...
        if option == ZMQ_CONNECT_ROUTING_ID {
            ...
            self.connect_routing_id = unsafe { String::from_raw_parts(optval.as_mut_ptr(), optvallen, optvallen) };
            return Ok(());
        }
    ...
    pub fn getsockopt(&mut self, ctx: &mut ZmqContext, options: &mut ZmqOptions, option_: u32) -> Result<Vec<u8>, ZmqError> {
        if self.ctx_terminated {
            // errno = ETERM;
            return Err(InvalidContext("Context was terminated"));
        }
        ...
        let mut result: Result<Vec<u8>, ZmqError>;
        match self.socket_type {
            ZmqSocketType::Client => {
        ...
        if option_ == ZMQ_EVENTS {
            self.process_commands(options, 0, false, ctx)?;
    ...
    pub fn join(&mut self, group_: &str) -> Result<(), ZmqError> {
    ...
    pub fn leave(&mut self, group_: &str) -> Result<(), ZmqError> {
        self.xleave(group_)
    }
    ...
    pub fn bind(&mut self, ctx: &mut ZmqContext, options: &ZmqOptions, endpoint_uri_: &str) -> Result<(), ZmqError> {
        if self.ctx_terminated {
    ...
    pub fn connect(&mut self, ctx: &mut ZmqContext, options: &mut ZmqOptions, endpoint_uri: &str) -> Result<(), ZmqError> {
    ...
    pub fn connect_internal(&mut self, ctx: &mut ZmqContext, options: &mut ZmqOptions, endpoint_uri: &str) -> Result<(), ZmqError> {
        ...
        if protocol == "udp" {
            ...
            paddr.udp_addr.resolve(options, &mut address, false,
                                   options.ipv6)?;
        }
        ...
        if options.immediate != 1 || subscribe_to_all {
            ...
            pipepair(parents, &mut new_pipes, hwms, conflates)?;
            // errno_assert (rc == 0);

            //  Attach local end of the pipe to the socket object.
            self.attach_pipe(ctx, options, new_pipes[0].unwrap(), subscribe_to_all, true);
    ...
    pub fn resolve_tcp_addr(&mut self, options: &ZmqOptions, endpoint_uri_pair_: &mut String, tcp_address_: &mut String) -> Result<String, ZmqError> {
        ...
        if self.endpoints.find(endpoint_uri_pair_) == self.endpoints.end() {
            ...
            tcp_addr.resolve(options, tcp_address_, false, options.ipv6)?;

            tcp_addr.to_string(endpoint_uri_pair_)?;
            if self.endpoints.find(endpoint_uri_pair_) == self.endpoints.end() {
                tcp_addr.resolve(options, tcp_address_, true, options.ipv6)?;
                tcp_addr.to_string(endpoint_uri_pair_)?;
            }
    ...
    pub fn term_endpoint(&mut self, ctx: &mut ZmqContext, options: &ZmqOptions, endpoint_uri_: &str) -> Result<(), ZmqError> {
    ...
    pub fn send(&mut self, ctx: &mut ZmqContext, options: &mut ZmqOptions, msg: &mut ZmqMsg, flags_: i32) -> Result<(), ZmqError> {
        ...
        //  Process pending commands, if any.
        self.process_commands(options, 0, true, ctx)?;
        ...
        loop {
            self.process_commands(options, timeout, false, ctx)?;
            self.xsend(ctx, options, msg)?;
            if timeout > 0 {
    ...
    pub fn recv(&mut self, ctx: &mut ZmqContext, options: &mut ZmqOptions, msg: &mut ZmqMsg, flags: i32) -> Result<(), ZmqError> {
        //  Check whether the context hasn't been shut down yet.
        if self.ctx_terminated {
            // errno = ETERM;
            return Err(SocketError("context was terminated"));
        ...
        loop {
            self.process_commands(options, if block { timeout } else { 0 }, false, ctx)?;
            self.xrecv(ctx, options, msg)?;
            // TODO
            // if rc == 0 {
            //     self.ticks = 0;
    ...
    pub fn process_commands(&mut self,
        ...
        self.mailbox.recv(&mut cmd, timeout_)?;
        let mut rc = 0;
        // TODO: process error for EINTR and continue
        ...
        while rc == 0 || get_errno() == EINTR {
            if rc == 0 {
                obj_process_command(options, &mut cmd, self.pipe.unwrap(), ctx)?;
            }
            self.mailbox.recv(&mut cmd, 0)?;
        }
    ...
    pub fn process_stop(&mut self, ctx: &mut ZmqContext, options: &mut ZmqOptions) -> Result<(), ZmqError> {
        self.stop_monitor(ctx, options, false)?;
        self.ctx_terminated = true;
        Ok(())
    }

    pub fn process_bind(&mut self,
                        ctx: &mut ZmqContext,
                        options: &mut ZmqOptions,
                        pipe: &mut ZmqPipe,
    ) -> Result<(), ZmqError> {
        self.attach_pipe(ctx, options, pipe, false, false)
    }

    pub fn process_term(&mut self, ctx: &mut ZmqContext, linger_: i32) {
        ...
        own_process_term(
            ctx,
            &mut self.own.owned,
            &mut self.own.terminating,
            &mut self.own.term_acks,
            linger_,
        );
    }

    pub fn process_term_endpoint(&mut self,
                                 ctx: &mut ZmqContext,
                                 options: &mut ZmqOptions,
                                 endpoint_: &String,
    ) -> Result<(), ZmqError> {
        self.term_endpoint(ctx, options, endpoint_)
    }
    ...
    pub fn query_pipes_stats(&mut self) -> Result<(), ZmqError> {
        {
    ...
    pub fn in_event(&mut self, ctx: &mut ZmqContext, options: &ZmqOptions) -> Result<(), ZmqError> {
        {
            // scoped_optional_lock_t sync_lock (_thread_safe ? &_sync : NULL);
    ...
    pub fn read_activated(&mut self, ctx: &mut ZmqContext, options: &mut ZmqOptions, pipe: &mut ZmqPipe) -> Result<(), ZmqError> {
    ...
    pub fn extract_flags(&mut self, msg_: &ZmqMsg) {
    ...
    pub fn monitor(&mut self,
        ...
        if self.monitor_socket.is_some() {
            self.stop_monitor(ctx, options, true)?;
        }
        ...
        zmq_setsockopt(options, &mut self.monitor_socket.unwrap(), ZMQ_LINGER as i32, &mut linger.to_le_bytes(), 4)?;

        //  Spawn the monitor socket endpoint
        if zmq_bind(ctx, options, &mut self.monitor_socket.unwrap(), endpoint).is_err() {
            self.stop_monitor(ctx, options, false)?;
    ...
    pub fn event_connected(&mut self,
                           ctx: &mut ZmqContext,
                           options: &mut ZmqOptions,
                           endpoint_uri_pair_: &ZmqEndpointUriPair,
                           err_: i32,
    ) {
    ...
    pub fn event_connect_delayed(&mut self,
                                 ctx: &mut ZmqContext,
                                 options: &mut ZmqOptions,
                                 endpoint_uri_pair_: &ZmqEndpointUriPair,
                                 err_: i32,
    ...
    pub fn event_connect_retried(&mut self,
                                 ctx: &mut ZmqContext,
                                 options: &mut ZmqOptions,
                                 endpoint_uri_pair_: &ZmqEndpointUriPair,
                                 err_: i32,
    ...
    pub fn event_listening(&mut self,
                           ctx: &mut ZmqContext,
                           options: &mut ZmqOptions,
                           endpoint_uri_pair_: &ZmqEndpointUriPair,
                           fd_: ZmqFd,
    ...
    pub fn event_bind_failed(&mut self,
                             ctx: &mut ZmqContext,
                             options: &mut ZmqOptions,
                             endpoint_uri_pair_: &ZmqEndpointUriPair,
                             err_: i32,
    ...
    pub fn event_accepted(&mut self,
                          ctx: &mut ZmqContext,
                          options: &mut ZmqOptions,
                          endpoint_uri_pair_: &ZmqEndpointUriPair,
                          fd_: ZmqFd,
    ...
    pub fn event_accept_failed(&mut self,
                               ctx: &mut ZmqContext,
                               options: &mut ZmqOptions,
                               endpoint_uri_pair_: &ZmqEndpointUriPair,
                               err_: i32,
    ...
    pub fn event_close_failed(&mut self,
                              ctx: &mut ZmqContext,
                              options: &mut ZmqOptions,
                              endpoint_uri_pair_: &ZmqEndpointUriPair,
                              err_: i32,
    ...
    pub fn event_disconnected(&mut self,
        ...
        let mut values: [u64; 1] = [fd_ as u64];
        self.event(ctx, options, endpoint_uri_pair_, &values, 1, ZMQ_EVENT_DISCONNECTED);
    }

    pub fn event_handshake_failed_no_detail(&mut self,
                                            ctx: &mut ZmqContext,
                                            options: &mut ZmqOptions,
                                            endpoint_pair_: &ZmqEndpointUriPair,
                                            err_: i32,
    ) {
        let mut values: [u64; 1] = [err_ as u64];
        self.event(
    ...
    pub fn event_handshake_failed_protocol(&mut self,
                                           ctx: &mut ZmqContext,
                                           options: &mut ZmqOptions,
                                           endpoint_pair_: &ZmqEndpointUriPair,
                                           err_: i32,
    ...
    pub fn event_handshake_failed_auth(&mut self,
                                       ctx: &mut ZmqContext,
                                       options: &mut ZmqOptions,
                                       endpoint_pair_: &ZmqEndpointUriPair,
                                       err_: i32,
    ...
    pub fn event_handshake_succeeded(&mut self,
                                     ctx: &mut ZmqContext,
                                     options: &mut ZmqOptions,
                                     endpoint_pair_: &ZmqEndpointUriPair,
                                     err_: i32,
    ...
    pub fn event(&mut self,
        ...
        if self.monitor_events & event_type {
            self.monitor_event(ctx, options, event_type, values_, values_count_, endpoint_uri_pair_)?;
        }
    }
        ...
        if self.monitor_socket {
            ...
            match options.monitor_event_version {
                1 => {
                    ...
                    zmq_msg_send(ctx, options, &mut msg, &mut self.monitor_socket.unwrap(), ZMQ_SNDMORE)?;
                    ...
                    zmq_msg_send(ctx, options, &mut msg, &mut self.monitor_socket.unwrap(), 0)?;
                }
                2 => {
    ...
    pub fn stop_monitor(&mut self,
                        ctx: &mut ZmqContext,
                        options: &mut ZmqOptions,
                        send_monitor_stopped_event_: bool,
    ) -> Result<(), ZmqError> {
        // this is a private method which is only called from
        // contexts where the _monitor_sync mutex has been locked before
    ...
    pub fn add_out_pipe(&mut self, routing_id_: &Vec<u8>, pipe_: &mut ZmqPipe) {
    ...
    pub fn has_out_pipe(&mut self, routing_id_: Vec<u8>) -> bool {
        let routing_id_u32 = u32::from_le_bytes([routing_id_[0], routing_id_[1], routing_id_[2], routing_id_[3]]);
        return self.out_pipes.contains_key(&routing_id_u32);
    }

    pub fn lookup_out_pipe(&mut self, routing_id_: Vec<u8>) -> &'a mut ZmqOutpipe<'a> {
    ...
    pub fn erase_out_pipe(&mut self, pipe_: &mut ZmqPipe) {
        let erased = self.out_pipes.erase(pipe_.get_routing_id());
    }

    pub fn try_erase_out_pipe(&mut self, routing_id_: &Vec<u8>) -> ZmqOutpipe {
        // out_pipes_t::iterator it = _out_pipes.find (routing_id_);
        // if (it == _out_pipes.end ())
        //     return out_pipe_t ();
