use crate::ctx::ZmqContext;
use crate::defines::err::ZmqError;
use crate::defines::err::ZmqError::SocketError;
use crate::defines::ZMQ_MSG_MORE;
use crate::msg::ZmqMsg;
use crate::pipe::ZmqPipe;
use crate::socket::ZmqSocket;

// pub struct ZmqDgram<'a> {
//     pub socket_base: ZmqSocket<'a>,
//     pub _pipe: Option<&'a mut ZmqPipe<'a>>,
//     pub _more_out: bool,
// }
//
// impl ZmqDgram {
//     pub unsafe fn new(options: &mut options_t,
//                       parent_: &mut ctx_t,
//                       tid_: u32,
//                       sid_: i32) -> ZmqDgram {
//         ZmqDgram {
//             socket_base: ZmqSocket::new(parent_, tid_, sid_, false),
//             _pipe: None,
//             _more_out: false,
//         }
//     }
//
//
// }

pub fn dgram_xattach_pipe(ctx: &mut ZmqContext, socket: &mut ZmqSocket, pipe_: &mut ZmqPipe, subscribe_to_all: bool, locally_initiated: bool) {
    if socket.pipe.is_none() {
        socket.pipe = Some(pipe_);
    } else {
        pipe_.terminate(ctx, false);
    }
}

pub fn dgram_xpipe_terminated(socket: &mut ZmqSocket, pipe_: &mut ZmqPipe) {
    if socket.pipe.is_some() && socket.pipe.unwrap() == pipe_ {
        socket.pipe = None;
    }
}

pub fn dgram_xread_activated(socket: &mut ZmqSocket, pipe_: &mut ZmqPipe) -> Result<(),ZmqError>{
    unimplemented!()
}

pub fn dgram_xwrite_activated(socket: &mut ZmqSocket, pipe_: &mut ZmqPipe) {
    unimplemented!()
}

pub fn xsend(ctx: &mut ZmqContext, socket: &mut ZmqSocket, msg_: &mut ZmqMsg) -> Result<(),ZmqError> {
    // If there's no out pipe, just drop it.
    if socket.pipe.is_none() {
        msg_.close()?;
        // errno_assert (rc == 0);
        return Err(SocketError("No pipe"));
    }

    //  If this is the first part of the message it's the ID of the
    //  peer to send the message to.
    if !socket.more_out {
        if !(msg_.flags() & ZMQ_MSG_MORE != 0u8) {
            // errno = EINVAL;
            return Err(SocketError("dgram xsend failed"));
        }
    } else {
        //  dgram messages are two part only, reject part if more is set
        if msg_.flags() & ZMQ_MSG_MORE {
            // errno = EINVAL;
            return Err(SocketError("dgram xsend failed"));
        }
    }

    // Push the message into the pipe.
    socket.pipe.unwrap().write(msg_)?;

    if !(msg_.flags() & ZMQ_MSG_MORE) {
        socket.pipe.unwrap().flush(ctx);
    }

    // flip the more flag
    socket.more_out = !socket.more_out;

    //  Detach the message from the data buffer.
    msg_.init2()?;
    // errno_assert (rc == 0);

    Ok(())
}

pub fn dgram_xrecv(ctx: &mut ZmqContext, socket: &mut ZmqSocket, msg_: &mut ZmqMsg) -> Result<(),ZmqError> {
    //  Deallocate old content of the message.
    msg_.close()?;
    // errno_assert (rc == 0);

    if socket.pipe.is_none() || socket.pipe.unwrap().read(ctx, msg_).is_err() {
        //  Initialise the output parameter to be a 0-byte message.
        msg_.init2()?;
        // errno_assert (rc == 0);

        // errno = EAGAIN;
        return Err(SocketError("dgram xrecv failed"));
    }

    Ok(())
}

pub  fn dgram_xhas_in(socket: &mut ZmqSocket) -> bool {
    if socket.pipe.is_none() {
        return false;
    }

    return socket.pipe.unwrap().check_read();
}

pub  fn dgram_xhas_out(socket: &mut ZmqSocket) -> bool {
    if socket.pipe.is_none() {
        return false;
    }

    return socket.pipe.unwrap().check_write();
}

pub fn dgram_xsetsockopt(socket: &mut ZmqSocket, option_: i32, optval_: &[u8], optvallen_: usize) -> Result<(),ZmqError> {
    unimplemented!()
}
pub fn dgram_xgetsockopt(socket: &mut ZmqSocket, option: u32) -> Result<Vec<u8>, ZmqError> {
    unimplemented!();
}

pub fn dgram_xjoin(socket: &mut ZmqSocket, group: &str) -> Result<(),ZmqError> {
    unimplemented!();
}

pub fn dgram_xsend(socket: &mut ZmqSocket, msg_: &mut ZmqMsg) -> Result<(),ZmqError> {
    unimplemented!()
}
