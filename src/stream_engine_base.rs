...
pub struct ZmqStreamEngineBase
{
    ...
    pub engine_interface: ZmqIEngine,
...
int ZmqStreamEngineBase::decode_and_push (msg: &mut ZmqMessage)
{
    // zmq_assert (_mechanism != null_mut());
    ...
    if (_session.push_msg (msg) == -1) {
        if (errno == EAGAIN)
            _process_msg = &ZmqStreamEngineBase::push_one_then_decode_and_push;
        return -1;
    }
    return 0;
}

int ZmqStreamEngineBase::push_one_then_decode_and_push (msg: &mut ZmqMessage)
{
...
int ZmqStreamEngineBase::pull_msg_from_session (msg: &mut ZmqMessage)
{
    return _session.pull_msg (msg);
}

int ZmqStreamEngineBase::push_ZmqMessageo_session (msg: &mut ZmqMessage)
{
    return _session.push_msg (msg);
}

void ZmqStreamEngineBase::// error (ZmqErrorReason reason_)
{
    // zmq_assert (_session);
...
void ZmqStreamEngineBase::set_handshake_timer ()
{
...
bool ZmqStreamEngineBase::init_properties (properties_t &properties_)
{
...
void ZmqStreamEngineBase::timer_event (id_: i32)
{
...
int ZmqStreamEngineBase::read (data: &mut [u8], size: usize)
{
...
int ZmqStreamEngineBase::write (const data: &mut [u8], size: usize)
{
