...
impl ZmqSessionBase {
    ...
    pub fn new(io_thread: &mut ZmqThread, active: bool, socket: &mut ZmqSocketBase, options: &ZmqOptions, addr: &mut Address) -> Self {
        let mut own = ZmqOwn::new(io_thread, options);
...
void ZmqSessionBase::hiccuped (ZmqPipe *)
{
...
int ZmqSessionBase::zap_connect ()
{
    ...
    ZmqPipe *new_pipes[2] = {null_mut(), null_mut()};
    int hwms[2] = {0, 0};
...
void ZmqSessionBase::process_attach (ZmqIEngine *engine_)
{
...
void ZmqSessionBase::engine_error (handshaked_: bool,
                                        ZmqIEngine::ZmqErrorReason reason_)
{
    ...
    // zmq_assert (reason_ == ZmqIEngine::connection_error
                || reason_ == ZmqIEngine::timeout_error
                || reason_ == ZmqIEngine::protocol_error);

    switch (reason_) {
        case ZmqIEngine::timeout_error:
            /* FALLTHROUGH */
        case ZmqIEngine::connection_error:
            if (active) {
                reconnect ();
                break;
            }

        case ZmqIEngine::protocol_error:
            if (_pending) {
                if (pipe)
