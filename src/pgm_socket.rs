...
//       ;[fe80::1%en0]:7500
    pub fn init_address (&mut self,  network_: &str, res: &mut &mut [pgm_addrinfo_t], port_number: &mut u16) -> i32
    {
        ...
        *port_number = u16::from_str_radix(&network_[port_delim+1..], 10)
        ...
        unsafe { copy_bytes(network.as_bytes_mut(), 0, network_.as_bytes(), 0, port_delim as i32); }
    ...
    pub fn init (&mut self, udp_encapsulation_: bool, network_: &str) -> i32
    {
        ...
        for i in 0 .. res.ai_recv_addrs_len
        {
            if (!pgm_setsockopt (sock, IPPROTO_PGM, PGM_JOIN_GROUP,
                                 &res.ai_recv_addrs[i], sizeof (group_req))){
        // goto err_abort;
        }
        }
        if (!pgm_setsockopt (sock, IPPROTO_PGM, PGM_SEND_GROUP,
                             &res.ai_send_addrs[0], sizeof (group_req))){
        // goto err_abort;
    }
...
//  waiting_pipe_fd_ is signaled for state driven events and data.
    pub fn get_receiver_fds (&mut self, receive_fd: ZmqFileDesc,
                             waiting_pipe_fd: ZmqFileDesc)
    {
        // socklen_t socklen;
        // rc: bool
...
//  pending_notify_fd_ is for state driven events.
    pub fn get_sender_fds (&mut self, send_fd_: &mut ZmqFileDesc,
