...
pub type ZmqFdEntries<'a> = Vec<fd_entry_t<'a>>;

pub struct family_entry_t<'a> {
...
pub struct ZmqSelect<'a> {
    ...
    #[cfg(not(target_os = "windows"))]
    pub _family_entry: family_entry_t<'a>,
    #[cfg(not(target_os = "windows"))]
    pub _max_fd: ZmqFd,
    #[cfg(target_os = "windows")]
...
impl<'a> ZmqSelect<'a> {
    ...
    pub fn rm_fd(&mut self, handle_: &mut ZmqHandle) {
        ...
        #[cfg(not(target_os = "windows"))]
        {
            let mut fd_entry_it = self.find_fd_entry_by_handle(&mut self._family_entry.fd_entries, handle_);

            self._family_entry.fds_set.remove_fd(&fd_entry_it.fd);
            fd_entry_it.fd = -1 as ZmqFd;
            retired += 1;
