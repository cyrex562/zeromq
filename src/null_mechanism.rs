...
use bincode::options;
use libc::{EAGAIN, EFAULT, EPROTO, memcmp, memcpy, read};
...
impl ZmqNullMechanism {
    ...
    pub fn new(session: &mut ZmqSessionBase, peer_address_: &str, options: &mut ZmqOptions) -> ZmqNullMechanism {
        ...
        if (_zap_reply_received && status_code != "200") {
            ...
            if (status_code != "300") {
               ...
               let msg_data = (msg.data_mut());
                copy_bytes(msg_data, offset, error_command_name, 0, error_command_name_len);

                // msg_data += error_command_name_len;
                offset += error_command_name_len;
                copy_bytes(msg_data, offset, &[(status_code_len as u8)], 0, error_reason_len_size);
                // *msg_data = status_code_len;
                // msg_data += error_reason_len_size;
                offset += error_reason_len_size;
                copy_bytes (msg_data, offset, status_code, 0,status_code_len);
                return 0;
            }
            errno = EAGAIN;
            return -1;
    ...
    pub fn process_ready_command (&mut self,
     cmd_data_: &mut [u8], data_size_: usize) -> i32
    {
...
pub fn process_error_command (&mut self,
cmd_data_: &mut [u8], data_size_: usize) -> i32
{
