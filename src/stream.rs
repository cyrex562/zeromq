...
use crate::context::{set_opt_bool, ZmqContext};
...
pub fn stream_xpipe_terminated(sock: &mut ZmqSocket, pipe: &mut ZmqPipe) {
    erase_out_pipe(pipe);
...
pub fn stream_xread_activated(sock: &mut ZmqSocket, pipe: &mut ZmqPipe) {
    sock.fair_queue.activated(pipe);
}

pub fn stream_xsend(sock: &mut ZmqSocket, msg: &mut ZmqMessage) -> i32 {
...
pub fn stream_xsetsockopt(sock: &mut ZmqSocket, opt_kind: i32,
    ...
    match (opt_kind) {
        ...
        // _ =>
        _ => {
            return sock.xsetsockopt(opt_kind, optval_);
        }
    }
}

pub fn  stream_xrecv(sock: &mut ZmqSocket, msg: &mut ZmqMessage) -> i32 {
    if sock._prefetched {
        if !sock._routing_id_sent {
    ...
    let rc = sock.fair_queue.recvpipe(&sock._prefetched_msg, &pipe);
    if (rc != 0) {
        return -1;
    }
    ...
    let metadata = sock._prefetched_msg.metadata();
    if (metadata) {
...
pub fn stream_xhas_in(sock: &mut ZmqSocket) -> bool {
    ...
    let routing_id = pipe.get_routing_id();
    rc = sock._prefetched_routing_id.init_size(routing_id.size());
    // errno_assert (rc == 0);

    // forward metadata (if any)
    let metadata = sock._prefetched_msg.metadata();
    if (metadata) {
        sock._prefetched_routing_id.set_metadata(metadata);
    }

    copy_bytes(sock._prefetched_routing_id.data_mut(), 0, routing_id.data(),
               0, routing_id.size());
    sock._prefetched_routing_id.set_flags(ZMQ_MSG_MORE);
...
pub fn stream_xhas_out(sock: &mut ZmqSocket) -> bool {
...
pub fn stream_identify_peer(sock: &mut ZmqSocket, pipe: &mut ZmqPipe, locally_initiated_: bool) {
    ...
    if locally_initiated_ && connect_routing_id_is_set() {
        ...
        put_u32(&mut buffer[1..], 0, sock._next_integral_routing_id);
