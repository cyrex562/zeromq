...
impl <T> GenericMtrie<T>
{
    ...
    pub unsafe fn add(&mut self, mut prefix_: Option<Prefix>, mut size_: usize, pipe: *mut T) -> bool
    {
        let mut it = self;
        while size_ {
            let c = *prefix_;
            ...
            if c < it._min || c > it._min + it._count {
                if !it._count {
                    ...
                    it._next.table = libc::malloc(it._count as usize * std::mem::size_of::<*mut GenericMtrie<T>>()) as *mut *mut GenericMtrie<T>;
                    ...
                    it._next.table = libc::realloc(it._next.table as *mut c_void, it._count as usize * std::mem::size_of::<*mut GenericMtrie<T>>()) as *mut *mut GenericMtrie<T>;
                    ...
                    it._next.table = libc::realloc(it._next.table as *mut c_void, it._count as usize * std::mem::size_of::<*mut GenericMtrie<T>>()) as *mut *mut GenericMtrie<T>;
                    libc::memmove(it._next.table.add((it._min - c) as usize) as *mut c_void, it._next.table as *const c_void, (old_count * std::mem::size_of::<*mut GenericMtrie<T>>()) as size_t);
            ...
            if it._count == 1 {
                ...
                if it._next.table[(c - it._min) as usize] == null_mut() {
                    it._next.table[(c - it._min) as usize] = Box::into_raw(Box::new(GenericMtrie::<T>::new()));
                    it._live_nodes += 1;
                }
