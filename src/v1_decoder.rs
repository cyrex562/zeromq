...
// }
// int zmq::v1_decoder_t::one_byte_size_ready (unsigned char const *)
pub unsafe fn v1d_one_byte_size_ready(decoder: &mut ZmqDecoder, buf: &[u8]) -> i32 {
    //  First byte of size is read. If it is UCHAR_MAX (0xff) read 8-byte size.
    //  Otherwise allocate the buffer for message data and read the
    //  message data into it.
    if (*decoder._tmpbuf == u8::MAX) {
        decoder.next_step(decoder._tmpbuf, 8, v1d_eight_byte_size_ready);
    } else {
        //  There has to be at least one byte (the flags) in the message).
        if (!*decoder._tmpbuf) {
            // errno = EPROTO;
        ...
        // assert (rc == 0);
        rc = decoder._in_progress.init_size(*decoder._tmpbuf - 1);
        if (rc != 0) {
            // errno_assert (errno == ENOMEM);
        ...
        decoder.next_step(decoder._tmpbuf, 1, v1d_flags_ready);
    }
    return 0;
}
...
//     }
pub fn msg(decoder: &mut ZmqDecoder) -> &mut ZmqMsg {
    decoder._in_progress.refm()
}
...
// }
// int zmq::v1_decoder_t::eight_byte_size_ready (unsigned char const *)
pub unsafe fn v1d_eight_byte_size_ready(decoder: &mut ZmqDecoder, buf: &[u8]) -> i32 {
    //  8-byte payload length is read. Allocate the buffer
    //  for message body and read the message data into it.
    let payload_length = get_u64(decoder._tmpbuf.as_ptr());
...
// #endif
    ...
    let mut rc = decoder._in_progress.close();
    // assert (rc == 0);
    rc = decoder._in_progress.init_size(msg_size);
    if (rc != 0) {
        // errno_assert (errno == ENOMEM);
        rc = decoder._in_progress.init2();
        // errno_assert (rc == 0);
...
//     return 0;
// }
pub unsafe fn v1d_flags_ready(decoder: &mut ZmqDecoder, buf: &[u8]) -> i32 {
    //  Store the flags from the wire into the message structure.
    decoder._in_progress.set_flags(decoder._tmpbuf[0] & ZmqMsg::more);
...
//     return 1;
// }
pub unsafe fn v1d_message_ready(decoder: &mut ZmqDecoder, buf: &[u8]) -> i32 {
    //  Message is completely read. Push it further and start reading
    //  new message. (in_progress is a 0-byte message after this point.)
    decoder.next_step(decoder._tmpbuf, 1, v1d_one_byte_size_ready);
    return 1;
}
