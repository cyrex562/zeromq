...
impl zap_client_t {
    ...
    pub unsafe fn send_zap_request(&mut self, options: &mut options_t, mechanism_: &str, credentials_: &[u8]) {
        let credential_list = [credentials_];
        self.send_zap_request2(options, mechanism_, &credential_list)
    }

    pub unsafe fn send_zap_request2(&mut self, options: &options_t, mechanism_: &str, credentials_: &[&[u8]]) {
    ...
    pub unsafe fn receive_and_process_zap_reply(&mut self) -> i32 {
        ...
        let mut msg: Vec<msg_t> = Vec::with_capacity(zap_reply_frame_count);
        ...
        for i in 0 .. zap_reply_frame_count
        {
            rc = self.mechanism_base.session.read_zap_msg (&mut msg[i]);
            if (rc == -1) {
                // if (errno == EAGAIN) {
                //     return 1;
                // }
        ...
        if (msg[0].size () > 0) {
            ...
            self.mechanism_base.session.get_socket ().event_handshake_failed_protocol (
              self.mechanism_base.session.get_endpoint (), ZMQ_PROTOCOL_ERROR_ZAP_UNSPECIFIED);
            // errno = EPROTO;
            // return close_and_return (msg, -1);
            return -1;
        }
        ...
        if (msg[3].size () != 3 || status_code_data[0] < '2'
            ...
            self.mechanism_base.session.get_socket ().event_handshake_failed_protocol (
              self.mechanism_base.session.get_endpoint (), ZMQ_PROTOCOL_ERROR_ZAP_INVALID_STATUS_CODE);
            // errno = EPROTO;
            // return close_and_return (msg, -1);
            return -1;
        }
        ...
        if (rc != 0) {
            self.mechanism_base.session.get_socket ().event_handshake_failed_protocol (
              self.mechanism_base.session.get_endpoint (), ZMQ_PROTOCOL_ERROR_ZAP_INVALID_METADATA);
            // errno = EPROTO;
            // return close_and_return (msg, -1);
            return -1;
        }
    ...
    pub unsafe fn handle_zap_status_code(&mut self)
    {
        ...
        self.mechanism_base.session.get_socket ().event_handshake_failed_auth (
          self.mechanism_base.session.get_endpoint (), status_code_numeric, "");
    }
}
...
impl zap_client_common_handshake_t {
    pub fn new(session_: &mut session_base_t, peer_address_: &str, options_: &options_t, zap_reply_ok_state_: zap_client_state) -> Self {
        Self {
    ...
    pub fn status(&mut self) -> mechanism::status_t {
        if self.state == ready {
            return mechanism::status_t::ready;
        }
        if self.state == error_sent {
            return mechanism::status_t::error;
        }
        return mechanism::status_t::handshaking;
    }
