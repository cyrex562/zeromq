...
{
    let mut on: c_int = 1;
    let rc = setsockopt(s_ as SOCKET, IPPROTO_IPV6 as c_int, IPV6_V6ONLY, &on as *const c_char, mem::size_of_val(&on) as c_int);
}
...
pub unsafe fn get_peer_ip_address(sockfd_: fd_t, ip_addr: &str) ->  i32
{
    ...
    let mut rc = getnameinfo(&ss as *const SOCKADDR, addrlen, Some(&mut host), None, NI_NUMERICHOST as i32);
...
pub unsafe fn set_ip_type_of_service(s_: fd_t, iptos_: i32)
{
    ...
    let rc = setsockopt(s_ as SOCKET, IPPROTO_IP as c_int, IP_TOS, &tos as *const c_char, mem::size_of_val(&tos) as c_int);
...
pub unsafe fn set_socket_priority(s_: fd_t, priority: i32) {
    let rc = setsockopt(s_, SOL_SOCKET, SO_PRIORITY, &priority as *const c_char, mem::size_of_val(&priority) as c_int);
}
...
pub unsafe fn set_nosigpipe(s_: fd_t)-> i32
{
    ...
    let mut on: c_int = 1;
    let rc = setsockopt(s_ as SOCKET, SOL_SOCKET, SO_NOSIGPIPE, &on as *const c_char, mem::size_of_val(&on) as c_int);
    return rc;
}

pub unsafe fn bind_to_device(s_: fd_t, bound_device_: &str) -> i32 {
    let rc = setsockopt(s_, SOL_SOCKET, SO_BINDTODEVICE, bound_device_.as_ptr() as *const c_char, bound_device_.len() as c_int);
    return rc;
}
...
pub unsafe fn shutdown_network() {
    #[cfg(target_os="windows")]
    {
        let rc = WSACleanup();
    }
...
{
    let tcp_nodelay = 1;
    let rc = setsockopt(socket_, IPPROTO_TCP, TCP_NODELAY, &tcp_nodelay as *const c_char, mem::size_of_val(&tcp_nodelay) as c_int);
