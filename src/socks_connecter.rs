...
use crate::socks::{
    ...
    ZmqSocksGreetingEncoder, ZmqSocksRequestEncoder, ZmqSocksResponseDecoder,
};
use crate::socks_connecter::ZmqSocksConnectorState::{
    sending_basic_auth_request, sending_greeting, sending_request, unplugged,
    waiting_for_auth_response, waiting_for_choice, waiting_for_proxy_connection,
...
use bincode::options;
use libc::{
    atoi, close, getsockopt, uint16_t, EHOSTUNREACH, EINPROGRESS, EINTR, EINVAL, ENETDOWN,
    ENETUNREACH, ETIMEDOUT,
};
use serde::__private::de::Content::String;
use std::ptr::null_mut;
use windows::Win32::Networking::WinSock::{
    socklen_t, WSAGetLastError, SOL_SOCKET, SO_ERROR, WSAEACCES, WSAEADDRINUSE, WSAECONNABORTED,
...
impl ZmqSocksConnector {
    pub fn new(
        ...
        let mut out = Self {
            connector_base: StreamConnecterBase::new(
                io_thread_,
                session,
                options,
                addr_,
    ...
    pub fn set_auth_method_basic(&mut self, username_: &str, password_: &str) {
    ...
    pub fn in_event(&mut self) {
        ...
        if (_status == waiting_for_choice) {
            ...
            // error ();
                ...
                let auth_response = _auth_response_decoder.decode();
            ...
            // error ();
                ...
                // error ();
                    ...
                    rm_handle();
                    create_engine(_s, get_socket_name(_s, SocketEndLocal));
                    _s = -1;
                    _status = unplugged;
                }
            }
        ...
        if (expected_status == sending_basic_auth_request) {
            _basic_auth_request_encoder
                .encode(ZmqSocksBasicAuthRequest(_auth_username, _auth_password));
    ...
    pub fn process_server_response(&mut self, response_: &ZmqSocksChoice) -> i32 {
        return if response_.method == socks_no_auth_required || response_.method == socks_basic_auth
        {
            0
        } else {
            -1
    ...
    pub fn process_server_response2(response_: &ZmqSocksResponse) -> i32 {
        return if response_.response_code == 0 { 0 } else { -1 };
    }

    pub fn process_server_response3(&mut self, response_: &ZmqSocksAuthResponse) -> i32 {
        return if response_.response_code == 0 { 0 } else { -1 };
    }
...
pub fn parse_address(address_: &str, hostname_: &mut str, mut port_: u16) -> i32 {
