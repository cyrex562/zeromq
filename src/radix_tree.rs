...
impl node_t {
    ...
    pub unsafe fn refcount(&mut self) -> u32 {
        let mut x = 0u32;
        libc::memcpy(&mut x as *mut c_void, self._data as *const c_void, std::mem::size_of::<u32>());
        x
    }

    pub unsafe fn set_refcount(&mut self, x: u32) {
        libc::memcpy(self._data as *mut c_void, &x as *const c_void, std::mem::size_of::<u32>());
    }

    pub unsafe fn prefix_len(&mut self) -> u32 {
        let mut x = 0u32;
        libc::memcpy(&mut x as *mut c_void, (self._data as *const c_void).offset(std::mem::size_of::<u32>() as isize), std::mem::size_of::<u32>());
        x
    }

    pub unsafe fn set_prefix_len(&mut self, x: u32) {
        libc::memcpy((self._data as *mut c_void).offset(std::mem::size_of::<u32>() as isize), &x as *const c_void, std::mem::size_of::<u32>());
    }

    pub unsafe fn edgecount(&mut self) -> u32 {
        let mut x = 0u32;
        libc::memcpy(&mut x as *mut c_void, (self._data as *const c_void).offset(std::mem::size_of::<u32>() as isize * 2), std::mem::size_of::<u32>());
        x
    }

    pub unsafe fn set_edgecount(&mut self, value_: u32) {
        libc::memcpy((self._data as *mut c_void).offset(std::mem::size_of::<u32>() as isize * 2), &value_ as *const c_void, std::mem::size_of::<u32>());
    }
    ...
    pub unsafe fn set_prefix(&mut self, value_: *const u8) {
        libc::memcpy((self._data as *mut c_void).offset(std::mem::size_of::<u32>() as isize * 3), &value_ as *const c_void, std::mem::size_of::<*mut u8>());
    }

    pub unsafe fn first_bytes(&mut self) -> *mut u8 {
        (self._data as *mut c_void).offset(std::mem::size_of::<u32>() as isize * 3 + self.prefix_len() as isize) as *mut u8
    }

    pub unsafe fn set_first_bytes(&mut self, value_: *mut u8) {
        libc::memcpy((self._data as *mut c_void).offset(std::mem::size_of::<u32>() as isize * 3 + self.prefix_len() as isize), &value_ as *const c_void, std::mem::size_of::<*mut u8>());
    }

    pub unsafe fn first_byte_at(&mut self, index_: usize) -> u8 {
        let mut x = 0u8;
        libc::memcpy(&mut x as *mut c_void, (self._data as *const c_void).offset(std::mem::size_of::<u32>() as isize * 3 + self.prefix_len() as isize + index_ as isize), std::mem::size_of::<u8>());
        x
    }

    pub unsafe fn set_first_byte_at(&mut self, index_: usize, byte_: u8) {
        libc::memcpy((self._data as *mut c_void).offset(std::mem::size_of::<u32>() as isize * 3 + self.prefix_len() as isize + index_ as isize), &byte_ as *const c_void, std::mem::size_of::<u8>());
    }
    ...
    pub unsafe fn add(&mut self, key_: *const u8, key_size_: usize) -> bool {
        ...
        if (key_bytes_matched != key_size_) {
            ...
            if (prefix_bytes_matched == current_node.prefix_length ()) {
                ...
                libc::memmove (current_node.node_pointers () as *mut c_void,
                ...
                current_node.set_edge_at ((current_node.edgecount() - 1) as usize,
            ...
            let mut key_node = make_node (1, key_size_ - key_bytes_matched, 0);
            let mut split_node =
              make_node (current_node.refcount() as usize,
    ...
    pub unsafe fn rm(&mut self, key_: *const u8, key_size_: usize) -> bool {
        ...
        if outgoing_edges == 1 {
           ...
           let old_prefix_length = current_node.prefix_length ();
            ...
            libc::memcpy (current_node.prefix ().add(old_prefix_length) as *mut c_void, child.prefix () as *const c_void,
                    child.prefix_length ());
        ...
        if (parent_node.edgecount () == 2 && parent_node.refcount () == 0
            ...
            // Append the child node's prefix to the current node.
            libc::memcpy (parent_node.prefix () + old_prefix_length,
                    other_child.prefix () as *const c_void, other_child.prefix_length ());
        ...
        // overwriting it.
        libc::memmove (parent_node.node_pointers ().sub(1) as *mut c_void, parent_node.node_pointers () as *const c_void,
