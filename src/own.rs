...
impl ZmqOwn {
    ...
    pub unsafe fn from_io_thread(io_thread_: *mut ZmqIoThread) -> Self {
        Self {
...
pub fn own_process_term(owned: &mut HashSet<&mut ZmqOwn>, terminating: &mut bool, term_acks: &mut i32, linger_: i32)
{
...
pub fn own_unregister_term_ack(terminating: &mut bool, processed_seqnum: &mut u64, sent_seqnum: &mut ZmqAtomicCounter, term_acks: &mut i32, owner: &mut Option<&mut ZmqOwn>)
{
...
pub fn own_process_term_ack(terminating: &mut bool, processed_seqnum: &mut u64, sent_seqnum: &mut ZmqAtomicCounter, term_acks: &mut i32, owner: &mut Option<&mut ZmqOwn>)
{
    own_unregister_term_ack (terminating, processed_seqnum, sent_seqnum, term_acks, owner);
}

pub fn own_check_term_acks(terminating: &mut bool, processed_seqnum: &mut u64, sent_seqnum: &mut ZmqAtomicCounter, term_acks: &mut i32, owner: &mut Option<&mut ZmqOwn>)
{
    if *terminating && processed_seqnum == sent_seqnum.get() as u64
        && term_acks == 0 {
