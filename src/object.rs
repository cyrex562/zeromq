...
impl ZmqObject {
    ...
    pub unsafe fn send_activate_write(&mut self, destination: &mut ZmqPipe, msgs_read: u64) {
        let mut cmd = ZmqCommand::new();
        cmd.destination = Some(destination);
    ...
    pub unsafe fn send_term_endpoint(&mut self, destination: &mut ZmqOwn, endpoint: &str) {
        let mut cmd = ZmqCommand::new();
