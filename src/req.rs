...
impl req_t {
    pub fn new(options: &mut options_t, parent_: &mut ctx_t, tid_: u32, sid_: i32) -> Self {
        options.type_ = ZMQ_REQ;

        Self {
            dealer: dealer_t::new(options, parent_, tid_, sid_),
            _request_id: 0,
            _strict: false,
        }
    }
    ...
    pub unsafe fn xsend(&mut self, msg_: &mut msg_t) -> i32 {
        ...
        let more = (msg_.flags() & msg_t::more) != 0;
    ...
    pub unsafe fn recv_reply_pipe(&mut self, msg_: &mut msg_t) -> i32 {
        loop {
            let mut pipe: pipe_t = pipe_t::default();
...
impl req_session_t {
    pub fn new(io_thread_: &mut io_thread_t, connect_: bool, socket_: &mut socket_base_t, options_: &options_t, addr_: address_t) -> Self {
        Self {
