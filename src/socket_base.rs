...
use crate::defines::{
    ...
    ZMQ_EVENT_ACCEPTED, ZMQ_EVENT_BIND_FAILED, ZMQ_EVENT_CLOSE_FAILED, ZMQ_EVENT_CLOSED,
    ZMQ_EVENT_CONNECT_DELAYED, ZMQ_EVENT_CONNECT_RETRIED, ZMQ_EVENT_CONNECTED,
    ZMQ_EVENT_DISCONNECTED, ZMQ_EVENT_HANDSHAKE_FAILED_AUTH, ZMQ_EVENT_HANDSHAKE_FAILED_NO_DETAIL,
    ZMQ_EVENT_HANDSHAKE_FAILED_PROTOCOL, ZMQ_EVENT_HANDSHAKE_SUCCEEDED,
    ZMQ_EVENT_LISTENING, ZMQ_EVENT_MONITOR_STOPPED, ZMQ_EVENT_PIPES_STATS,
    ZMQ_EVENTS, ZMQ_FD, ZMQ_IPV6, ZMQ_LAST_ENDPOINT, ZMQ_LINGER, ZMQ_POLLIN,
...
impl ZmqSocketBase {
    ...
    pub fn bind(&mut self, ctx: &mut ZmqContext, options: &mut ZmqOptions, endpoint_uri_: &str) -> anyhow::Result<()> {
        // scoped_optional_lock_t sync_lock (_thread_safe ? &sync : NULL);
        ...
        if self.parse_uri(endpoint_uri_, &mut protocol, &mut address) || self.check_protocol(options, &protocol) {
        ...
        if protocol == protocol_name::udp {
            ...
            let mut session = ZmqSessionBase::create(ctx, &mut io_thread.unwrap(), true, this, options, &mut paddr)?;
        ...
        if protocol == protocol_name::ws || protocol == protocol_name::wss {
            let listener = ws_listener_t::new(&mut io_thread, self, options, protocol == protocol_name::wss);
            // #else
            //     if protocol == protocol_name::ws {
            let listener = ws_listener_t::new(&mut io_thread, self, options, false);
            // #endif
            //         alloc_assert (listener);
            if listener.set_local_address(address.c_str()).is_err() {
                // LIBZMQ_DELETE (listener);
        ...
        if protocol == protocol_name::ipc {
            let mut listener = IpcListener::new(&mut io_thread.unwrap(), this, options);
            // alloc_assert (listener);
            if listener.set_local_address(address.c_str()).is_err() {
                // LIBZMQ_DELETE (listener);
                event_bind_failed(make_unconnected_bind_endpoint_pair(&address), zmq_errno());
    ...
    pub fn term_endpoint(&mut self, options: &mut ZmqOptions, endpoint_uri_: &str) -> i32 {
        ...
        if uri_protocol == protocol_name::inproc {
            return if self.unregister_endpoint(&endpoint_uri_str, self) == 0 {
                0
            } else {
    ...
    pub fn connect_internal(
        ...
        //  Create session.
        let mut session = ZmqSessionBase::create(ctx, &mut io_thread, true, self, options, &mut paddr);
        // errno_assert (session);

        //  PGM does not support subscription forwarding; ask for all data to be
        //  sent to this pipe. (same for NORM, currently?)
        // #if defined ZMQ_HAVE_OPENPGM && defined ZMQ_HAVE_NORM
        let subscribe_to_all = protocol == protocol_name::pgm || protocol == protocol_name::epgm || protocol == protocol_name::norm || protocol == protocol_name::udp;
        // #elif defined ZMQ_HAVE_OPENPGM
...
impl routing_socket_base_t {
    ...
    // {
    // }
    pub fn new(parent_: &mut ZmqContext, options: &mut ZmqOptions, tid: u32, sid_: i32) -> Self {
        Self {
            _out_pipes: HashMap::new(),
            base: ZmqSocketBase::new(parent_, options, tid, sid_, false),
