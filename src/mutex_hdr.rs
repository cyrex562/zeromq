...
impl mutex_t {
    ...
    // using pthreads and / or not windows
    // inline mutex_t ()
    // {
    // int rc = pthread_mutexattr_init ( & _attr); posix_assert (rc);
    //
    // rc = pthread_mutexattr_settype ( & _attr, PTHREAD_MUTEX_RECURSIVE); posix_assert (rc);
    //
    ...
    // inline ~mutex_t ()
    // {
    // int rc = pthread_mutex_destroy ( & _mutex); posix_assert (rc);
    //
    // rc = pthread_mutexattr_destroy ( & _attr); posix_assert (rc);
    // }

    // inline void lock ()
    // {
    // int rc = pthread_mutex_lock ( & _mutex); posix_assert (rc);
    // }

    // inline bool try_lock ()
    // {
    // int rc = pthread_mutex_trylock ( & _mutex); if (rc == EBUSY) return false;
    //
    // posix_assert (rc); return true;
    // }

    // inline void unlock ()
    // {
    // int rc = pthread_mutex_unlock ( & _mutex); posix_assert (rc);
    // }
