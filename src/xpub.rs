...
static void stub (mtrie_t::prefix_t data, size: usize, arg_: &mut [u8])
{
    LIBZMQ_UNUSED (data);
    LIBZMQ_UNUSED (size);
    LIBZMQ_UNUSED (arg_);
...
int xpub_t::xrecv (msg: &mut ZmqMessage)
{
    ...
    if (_manual && !_pending_pipes.empty ()) {
        ...
        if (_last_pipe != null_mut() && !_dist.has_pipe (_last_pipe)) {
            _last_pipe = null_mut();
        }
    }
