...
// bool xhas_out () ;
pub fn xpub_xhas_out() -> bool {
    return _dist.has_out();
}

// int xrecv (msg: &mut ZmqMessage) ;
pub fn xpub_xrecv(sock: &mut ZmqSocket, msg: &mut ZmqMessage) -> i32 {
    ...
    if (sock._manual && !sock._pending_pipes.empty()) {
        ...
        if (sock._last_pipe.is_some() && !sock._dist.has_pipe(sock: &mut ZmqSocket._last_pipe.unwrap())) {
            sock._last_pipe = None;
        }
    }
    ...
    let rc = msg.close();
    // errno_assert(rc == 0);
    rc = msg.init_size(sock._pending_data.front().size());
    // errno_assert(rc == 0);
    copy_bytes(
        msg.data(),
        0,
        ...
        *data = null_mut();
        ...
        if (is_subscribe_or_cancel) {
            ...
            if (sock._manual || (ctx.type_ == ZMQ_XPUB && notify)) {
                ...
                sock._pending_data.push_back(ZMQ_MOVE(notification));
                if (metadata) {
                    metadata.add_ref();
                }
            ...
            //  messages
            sock._pending_data.push_back(Blob(msg_data, msg.size()));
            if (metadata) {
                metadata.add_ref();
            }
            sock._pending_metadata.push_back(metadata);
...
pub fn xpub_xsetsockopt(sock: &mut ZmqSocket, option_: i32, optval_: &mut [u8], optvallen_: usize) -> i32 {
        ...
        || option_ == ZMQ_ONLY_FIRST_SUBSCRIBE)
    {
        ...
        if (sock._last_pipe != null_mut()) {
            sock._subscriptions
                .add(optval_, optvallen_, sock._last_pipe.clone());
        }
    } else if (option_ == ZMQ_UNSUBSCRIBE && sock._manual) {
        if (sock._last_pipe != null_mut()) {
            sock._subscriptions
                .rm(optval_, optvallen_, sock._last_pipe.clone());
        }
...
pub fn xpub_xgetsockopt(sock: &mut ZmqSocket, option_: i32, optval_: &mut [u8], optvallen_: *mut usize) -> i32 {
    if (option_ == ZMQ_TOPICS_COUNT) {
        ...
        return do_getsockopt(optval_, optvallen_, sock._subscriptions.num_prefixes());
    }
...
pub fn xpub_xpipe_terminated(sock: &mut ZmqSocket, pipe: &mut ZmqPipe) {
    if (sock._manual) {
        ...
        //  upstream.
        sock._subscriptions
            .rm(pipe, send_unsubscription, this, !sock._verbose_unsubs);
    }
...
pub fn send_unsubscription(sock: &mut ZmqSocket, data: TrieBuilder<Vec<u8>>, size: usize) {
    if (sock.options.type_ != ZMQ_PUB) {
        ...
        // }
        sock._pending_data.ZMQ_PUSH_OR_EMPLACE_BACK(ZMQ_MOVE(unsub));
