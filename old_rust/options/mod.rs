...
impl ZmqOptions {
    pub fn new() -> Self {
        let mut out = Self {
            ...
            curve_public_key: [0;32],
            curve_secret_key: [0;32],
            curve_server_key: [0;32],
    ...
    pub fn setsockopt(&mut self, option_: i32, optval_: &mut [u8], optvallen_: size_t) -> Result<(), ZmqError> {
        ...
        match option_ as u32 {
            ...
            ZMQ_TCP_ACCEPT_FILTER => {
                ...
                //     } else {
                //         let mut mask: TcpAddressMask = TcpAddressMask::default();
                //         if mask.resolve(options, filter_str.as_str(), self.ipv6).is_ok() {
                ...
                if filter_str.is_empty() {
                    ...
                    let mut mask: TcpAddressMask = TcpAddressMask::default();
                    if mask.resolve(self, filter_str.as_str(), self.ipv6).is_ok() {
                        self.tcp_accept_filters.push(mask);
                        // rc = 0;
                    }
                }
            ...
            ZMQ_OUT_BATCH_SIZE => {
                if is_int && value >= 0 {
                    self.out_batch_size = value as usize;
                    return Ok(());
                }
            }
            ZMQ_BUSY_POLL => {
    ...
    pub fn getsockopt(
        ...
        match option_ {
            ...
            ZMQ_ROUTING_ID => {
                ...
                return Ok(self.routing_id.to_vec());
            }
            ZMQ_RATE => {
            ...
            ZMQ_SOCKS_PROXY => {
                ...
                return Ok(self.socks_proxy_address.into_bytes());
            }
            // ZMQ_SOCKS_USERNAME => {
            //     // return do_getsockopt(optval_, optvallen_, &self.socks_username);
            //     return Ok(self.socks_username.into_bytes());
            // }
            // ZMQ_SOCKS_PASSWORD => {
            //     // return do_getsockopt(optval_, optvallen_, &self.socks_password);
            //     return Ok(self.socks_password.into_bytes());
            // }
            ZMQ_TCP_KEEPALIVE => {
            ...
            ZMQ_PLAIN_USERNAME => {
                ...
                return Ok(self.plain_username.into_bytes());
            }
            ZMQ_PLAIN_PASSWORD => {
                // return do_getsockopt(optval_, optvallen_, &self.plain_password);
                return Ok(self.plain_password.into_bytes());
            }
            ZMQ_ZAP_DOMAIN => {
                // return do_getsockopt(optval_, optvallen_, &self.zap_domain);
                return Ok(self.zap_domain.into_bytes());
            }
            ZMQ_CONFLATE => {
            ...
            //     return Ok(self.bindtodevice.into_bytes());
            // }
            ZMQ_ZAP_ENFORCE_DOMAIN => {
            ...
            ZMQ_MULTICAST_LOOP => {
                return Ok(self.multicast_loop.to_le_bytes().into_vec());
            }
            ZMQ_ROUTER_NOTIFY => {
...
pub fn get_effective_conflate_option(options: &ZmqOptions) -> bool {
    return options.conflate && (options.socket_type == ZMQ_DEALER || options.socket_type == ZMQ_PULL || options.socket_type == ZMQ_PUSH || options.socket_type == ZMQ_PUB || options.socket_type == ZMQ_SUB);
}

pub fn do_getsockopt<T>(value_: T) -> Result<Vec<u8>, ZmqError> {
    todo!()
